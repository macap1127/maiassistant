    ...
    return (
      <div className="min-h-[100dvh] bg-background flex items-start sm:items-center justify-center overflow-y-auto px-6 py-8" style={{ paddingTop: "max(2rem, env(safe-area-inset-top))", paddingBottom: "max(2rem, env(safe-area-inset-bottom))" }}>
        <div className="w-full max-w-sm text-center animate-fade-in">
          ...
          <h1 className="text-2xl font-serif font-semibold mb-3">{t("auth.checkYourEmail")}</h1>
        ...
        {mode === "forgot" && resetSent ? (
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              {t("auth.resetSentMessagePart1")} <span className="font-medium text-foreground break-all">{email}</span>{t("auth.resetSentMessagePart2")}
            </p>
            <button
          ...
          {mode === "signup" && (
            <label className="flex items-start gap-2.5 text-left text-sm text-muted-foreground cursor-pointer select-none px-1">
              ...
              <span>
                ...
                {" "}{t("auth.and")}{" "}
                <Link to="/terms" className="text-primary hover:underline">{t("auth.termsOfService")}</Link>{t("auth.agreeToSuffix")}
              </span>
            </label>
          )}
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  if (result.error) throw new Error(result.error.message || t("auth.googleSignInFailed"));
                  if (result.redirected) return;
                } catch (err: any) {
                  console.error("Google auth error:", err);
                  setError(err.message || t("auth.googleSignInFailedRetry"));
                  setLoading(false);
                }
              }}
              disabled={loading}
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setError(err.message || t("auth.appleSignInFailedRetry"));
                  setLoading(false);
                }
              }}
              disabled={loading}
